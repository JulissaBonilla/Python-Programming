...
def list_weather_violations(directory):
    ...
    for row in tablelessons[1:]:
        violation=''
        takeoff=utils.str_to_time(row[3],reportdaycycle['timezone'])
        landing=utils.str_to_time(row[4],reportdaycycle['timezone'])
        daytime=utils.daytime(takeoff,reportdaycycle) and utils.daytime(landing, reportdaycycle)
        studentid=utils.get_for_id(row[0],tablestudents)
        pilotcred=pilots.get_certification(takeoff,studentid)
        instructed=True if row[2]!=''else False
        pilotmin=pilots.get_minimums(pilotcred,row[6],instructed,row[5]=='VFR',daytime,tableminimums)
