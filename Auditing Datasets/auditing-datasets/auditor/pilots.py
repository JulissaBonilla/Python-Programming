...
def get_certification(takeoff,student):
    ...
    """
    certification = PILOT_INVALID
    joined = utils.str_to_time(student[3],takeoff.tzinfo)
    if (joined) and takeoff < joined:
        certification = PILOT_INVALID
    solo = utils.str_to_time(student[4],takeoff.tzinfo)
    if (joined) and takeoff > joined:
        certification = PILOT_NOVICE
    license = utils.str_to_time(student[5],takeoff.tzinfo)
    fiftyhrs = utils.str_to_time(student[6],takeoff.tzinfo)
    if (solo) and takeoff > solo:
        certification = PILOT_STUDENT
    if (license) and takeoff > license:
        certification = PILOT_CERTIFIED
    if  (fiftyhrs) and takeoff > fiftyhrs:
        certification = PILOT_50_HOURS
...
def has_instrument_rating(takeoff,student):
    ...
    Precondition: student is 10-element list of strings representing a pilot
    """
    instrument=utils.str_to_time(student[7],takeoff.tzinfo)
    if (instrument) and instrument<takeoff:
        return True
    else:
        return False
...
def has_advanced_endorsement(takeoff,student):
    ...
    Precondition: student is 10-element list of strings representing a pilot
    """
    advanced=utils.str_to_time(student[8],takeoff.tzinfo)
    if (advanced) and advanced<takeoff:
        return True
    else:
        return False
...
def has_multiengine_endorsement(takeoff,student):
    ...
    Precondition: student is 10-element list of strings representing a pilot
    """
    multiengine=utils.str_to_time(student[9],takeoff.tzinfo)
    if(multiengine) and multiengine<takeoff:
        return True
    else:
        return False
