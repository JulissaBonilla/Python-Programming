...
def sunset(date,daycycle):
    ...
    for x in daycycle:
        if x.isnumeric():
            if date.year==int(x):
                for y in daycycle[x]:
                    if date.month==int(y.split('-')[0]) and date.day == int(y.split('-')[1]):
                        return str_to_time(x + '-' + y.split('-')[0] + '-' + y.split('-')[1] + 'T' + daycycle[x][y]['sunset'].split(':')[0] + ':' + daycycle[x][y]['sunset'].split(':')[1])
    return None
