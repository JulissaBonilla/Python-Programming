...
"""
import introcs
APIKEY = "9y8ps21qmlBh8cPYuHj8Cp1ltM0PnybGlXQPzD49Se0e"
...
def first_inside_quotes(s):
    ...
    assert type(s) == str
    assert '"' in s
    quote1= introcs.find_str(s,'"')
    quote1=quote1+1
    assert '"' in s[quote1:]
    quote2= introcs.find_str((s[quote1:]),'"')
    return s[quote1:quote1+quote2]
...
def get_src(json):
    ...
    srcLoc= introcs.find_str(json,'"src":')
    srcLoc=srcLoc+5
    return first_inside_quotes(json[srcLoc+1:])
...
def get_dst(json):
    ...
    """
    assert type(json) == str
    dstLoc= introcs.find_str(json,'"dst":')
    dstLoc=dstLoc+5
    return first_inside_quotes(json[dstLoc+1:])
...
def has_error(json):
    ...
    """
    assert type(json) == str
    successLoc= introcs.find_str(json,'"success":')
    commaLoc=introcs.find_str(json,',')
    success=json[successLoc+10:commaLoc]
    success=introcs.strip(success)
    success=introcs.capitalize(success)
    return not eval(success)
...
def service_response(src,dst,amt):
    ...
    q=q+str(src)+'&dst='+str(dst)+'&amt='+str(amt)+'&key='+APIKEY
...
def iscurrency(currency):
    ...
    error= has_error(service_response(currency,'USD',1.0))
    return not error
...
def exchange(src,dst,amt):
    ...
    money= get_dst(json)
    money=introcs.strip(money,'abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ')
    return float(money)