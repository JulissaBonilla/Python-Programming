...
def session(bet):
    ...
    while(score<20):
        ...
        if choice=='a':
            score= int(score) + random.randint(4, 7)
        elif choice=='b':
            score= int(score) + random.randint(1, 8)
        elif choice=='s':
            break
        if score==20:
            break

    if payout(bet,score)==0:
        print("Your score is "+ str(score) +".")
        print("You won "+str(payout(bet,score))+" credits.")
    if payout(bet,score)>0 and payout(bet,score)!=bet :
        print("You won "+str(payout(bet,score))+" credits.")
    if payout(bet,score)<0and payout(bet,score)!=-bet:
        print('You lost '+ str(payout(bet,score))+' credits.')
    if payout(bet,score)==bet:
        print("Your score is "+ str(score) +".")
        print('Quasar!')
        print("You won "+str(payout(bet,score))+" credits.")
    if payout(bet,score)==-bet:
...
def play(credits):
    ...
    while credits>0:
        ...
        if credits==0:
            ...
            break
        print("You have "+str(credits)+" credits.")
        replay=prompt("Do you want to (c)ontinue or (p)ayout? ",('c','p'))
        if replay=='c':
            continue
        if replay=='p':
